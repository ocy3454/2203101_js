...
var mySqlClient = mysql.createConnection({
    connectionLimit: 10,
    host: 'database-1.c2zqm4e8bsu4.ap-northeast-1.rds.amazonaws.com',
    user: 'admin',
    password: 'xpelqpdj12',
    database: 'account',
    dateStrings: 'date',
...
router.route('/').get(function (req, res) {
    ...
    if (req.session.user) {
        fs.readFile('./public/todolist.html', 'utf8', function (error, data) {
            if (error) {
                console.dir(error);
            } else {
...
router.route('/ocy/go_register').get(function (req, res) {
...
//회원가입 라우터
router.route('/ocy/register').post(function (req, res) {
...
//로그인 라우터
router.route('/ocy/login').post(function (req, res) {
...
//로그아웃 라우터
router.route('/ocy/logout').get(function (req, res) {
    console.log('/ocy/logout 호출됨');
    if (req.session.user) {
        console.log('로그아웃함');
...
//To-Do 추가 라우터
router.route('/ocy/addtodo').post(function (req, res) {
...
//To-Do 데이터 전체삭제 라우터
router.route('/ocy/deleteall').get(function (req, res) {
...
//To-Do status 변경 라우터
router.route('/ocy/leftstat/').get(function (req, res) {
    var _url = req.url;
...
router.route('/ocy/rightstat/').get(function (req, res) {
    var _url = req.url;
    var queryData = url.parse(_url, true).query;
    var id = queryData.id;
    var stat = parseInt(queryData.stat) + 1;
...
//To-Do 삭제 라우터
router.route('/ocy/deltodo/:id').get(function (req, res) {
